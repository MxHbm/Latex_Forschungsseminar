...
According to \citeauthor*{bischoff_issues_1995}, the mere placement of items in containers is insufficient
if practical requirements are not fulfilled as it lacks applicability. \footcite[cf.][pp. 1--2]{bischoff_issues_1995}
\citeauthor*{bortfeldt_constraints_2013} systematically categorized all constraints emphasized in \gls{CLP} research
...
items are placed on the container floor. Two stacking approaches exist.
...
cannot be split. The allocation constraint serves a similar purpose,
including the connectivity constraint, which mandates that
certain items must be loaded into the same container, and the
separation constraint, which requires specific items to
...
maximum allowable distance reachable from the unloading point (\textit{reachability}). Figure~\ref{fig:solution-visualization}
